-- ============================================
-- Script Unificado para Aplicar Migrações CNAE
-- ============================================
-- Execute este script no Supabase Dashboard -> SQL Editor
-- OU via CLI: psql <connection_string> -f APLICAR_CNAE_MIGRATIONS.sql
--
-- Este script aplica:
-- 1. Criação das tabelas e colunas (20251106000001_create_cnae_tables.sql)
-- 2. População inicial de dados (20251106000002_seed_cnae_tributacao.sql)
-- ============================================

-- ============================================
-- PARTE 1: Criação de Tabelas e Colunas
-- ============================================

-- Adicionar colunas CNAE na tabela profiles
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'handle_updated_at') THEN
    DROP TRIGGER IF EXISTS handle_updated_at_cnae_tributacao ON public.cnae_tributacao;
    CREATE TRIGGER handle_updated_at_cnae_tributacao
      BEFORE UPDATE ON public.cnae_tributacao
      FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();

    DROP TRIGGER IF EXISTS handle_updated_at_codigos_tributacao_cache ON public.codigos_tributacao_cache;
...
DROP POLICY IF EXISTS "Allow read access to cnae_tributacao" ON public.cnae_tributacao;
CREATE POLICY "Allow read access to cnae_tributacao" ON public.cnae_tributacao
  FOR SELECT USING (true);

DROP POLICY IF EXISTS "Allow read access to codigos_tributacao_cache" ON public.codigos_tributacao_cache;
CREATE POLICY "Allow read access to codigos_tributacao_cache" ON public.codigos_tributacao_cache
  FOR SELECT USING (true);

-- ============================================
-- PARTE 2: População Inicial de Dados (Seed)
-- ============================================
...
SELECT 
  'Colunas CNAE em profiles' as verificacao,
  CASE 
    WHEN EXISTS (
...
SELECT 
  'Tabela cnae_tributacao' as verificacao,
  CASE 
    WHEN EXISTS (
      SELECT 1 FROM information_schema.tables 
      WHERE table_schema = 'public' 
      AND table_name = 'cnae_tributacao'
    ) THEN '✅ Criada'
    ELSE '❌ Não encontrada'
  END as status;

SELECT 
  'Tabela codigos_tributacao_cache' as verificacao,
  CASE 
    WHEN EXISTS (
      SELECT 1 FROM information_schema.tables 
      WHERE table_schema = 'public' 
      AND table_name = 'codigos_tributacao_cache'
    ) THEN '✅ Criada'
    ELSE '❌ Não encontrada'
  END as status;
...
SELECT 
  'Total de mapeamentos CNAE → cTribNac' as verificacao,
  COUNT(*) as total_registros
FROM public.cnae_tributacao;

-- Mostrar alguns exemplos
SELECT 
  'Exemplos de mapeamentos' as verificacao,
  cnae,
  codigo_tributacao,
  descricao_servico
FROM public.cnae_tributacao
