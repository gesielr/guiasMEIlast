...
class SupabaseService:
    ...
    def client(self):
        ...
        if self._client is None:
            try:
                ...
                print(f"⚠ AVISO: Problema ao conectar Supabase: {str(e)[:60]}...")
                print("  Sistema funcionará em modo limitado (sem persistência)")
                self._client = False  # Marcar como tentou
        return self._client if self._client else None

    async def create_record(self, table: str, data: Dict[str, Any]) -> Dict[str, Any]:
        if not self.client:
            print(f"⚠ Supabase não disponível - criando record em memória")
            return data
        ...
        try:
            ...
            print(f"✗ Erro ao criar record: {str(e)[:60]}...")
            return data
    ...
    async def get_records(self, table: str, filters: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"✗ Erro ao buscar records: {str(e)[:60]}...")
            return []

    async def upload_file(self, bucket: str, file_path: str, file_data: bytes, content_type: str = "application/pdf") -> str:
        if not self.client:
            print(f"⚠ Supabase não disponível - arquivo não será armazenado")
            return f"temp://{file_path}"
        
        try:
            def _upload():
                return self.client.storage.from_(bucket).upload(file_path, file_data, {"content-type": content_type})
            await asyncio.to_thread(_upload)
            def _get_public_url():
                return self.client.storage.from_(bucket).get_public_url(file_path)
            public_url = await asyncio.to_thread(_get_public_url)
            if not public_url:
                raise RuntimeError("Não foi possível obter URL pública do arquivo")
            return public_url
        except Exception as e:
            print(f"✗ Erro ao fazer upload: {str(e)[:60]}...")
            return f"temp://{file_path}"

    async def obter_usuario_por_whatsapp(self, whatsapp: str) -> Optional[Dict[str, Any]]:
        """Obtém usuário pelo número de WhatsApp."""
        if not self.client:
            print(f"⚠ Supabase não disponível - retornando None")
            return None
        ...
        try:
            ...
            print(f"✗ Erro ao obter usuário: {str(e)[:60]}...")
            return None

    async def criar_usuario(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Cria novo usuário."""
        if not self.client:
            print(f"⚠ Supabase não disponível - retornando dados em memória")
            # Retornar dados com ID mockado para testes
            return {**data, "id": f"mock-{data.get('whatsapp', 'unknown')}"}
        
        try:
            return await self.create_record("usuarios", data)
        except Exception as e:
            print(f"✗ Erro ao criar usuário: {str(e)[:60]}...")
    ...
    async def salvar_guia(self, user_id: str, guia_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not self.client:
            print(f"⚠ Supabase não disponível - guia não será persistida")
